...
function divcreator(idname, where) {
    ...
    if (where == "body") {
        ...
        if (0 < iDiv.id.search("day_created")) {
            document.getElementById(iDiv.id).setAttribute('style', 'margin: 22px 0px 22px 0px; width: 100%; color: lightgrey;text-shadow: 2px 2px #484848;border-bottom: #1b1b1b solid 2px;');
            if (themeStatus == "light") {
                document.getElementById(iDiv.id).style.color = "black";
            }
        }
...
function HtmlStart() {
    ...
    for (var i = 0; i < eventsLength; i++) {
        ...
        if ((liveTimestamp < streamEnd[i] + 3000) & (liveTimestamp > streamStart[i] - 3000) & (elapsed > 0) & (startTime[0] == endTime[0])) {
            document.getElementById(timeId + "_created").innerHTML = startTime[0] + "<br>" + startTime[1] + "-" + endTime[1] + "<font color=\"yellow\"> + " + elapsed + "p</font>";
...
function OfflineSite() {
    ...
    for (var i = 0; i < titles.length; i++) {
        ...
        if (startTime[0] == endTime[0]) {
            if ((countdownStart < 7200) & (countdownStart > 0)) { Countdown(streamStart[i]); } else { document.getElementById(timeId + "_created").innerHTML = startTime[0] + "<br>" + startTime[1] + "-" + endTime[1]; }
        } else {
