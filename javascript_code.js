...
var NewScriptMessage ="Megérkezetett hozzád az új "+scriptVersion+" amely az alábbi új funkciókat tartalmazza:<br>";
NewScriptMessage += "Menetrendi események napok szerinti kategorizálása a még kényelmesebb használatért!"
...
function DayDivider(streamStartT, currentT, i) {
    ...
    if ((sn == cn) & (StreamWeekStart == CurrentWeekStart)) {
        ...
        if (((sn - cn) == 1) & (StreamWeekStart == CurrentWeekStart)) { streamDay[i] = "Holnap"; }
        if (weekDistance == 0) { }
        if (weekDistance == 1) { streamDay[i] = "Jövőhét " + streamDay[i] }
        //if (weekDistance == 2) { streamDay[i] = "Jövőhét Utáni " + streamDay[i] }
...
function HtmlStart() {
    ...
    var oldScriptVersion = getCookie(streamer + "_scriptversion");

    if ((cookieSettings == 1) & (scriptVersion != oldScriptVersion) & (curentUserID != 0)) {
...
function modal_open(i) {
    ...
    if (i == "newScriptLoaded") {
        var formated = "<div style=\"max-width:400px\" align=\"left\"><font size=\"2\">"+NewScriptMessage+"</font></div><br><br>";
        popupContent.innerHTML = "<br><br><span style=\"color: red\"><b>[Új Funkciók érkeztek!]</b></span><br><br>" + formated;
        createcookie(streamer + "_scriptversion", scriptVersion, 365);
    }
}
