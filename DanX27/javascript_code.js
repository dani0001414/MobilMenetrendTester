...
window.onerror = function (msg, u, l, columnNo) {
    ...
    var parser, browser, browserText, flag = 1;
    parser = new UAParser();

    var os = parser.setUA(navigator.userAgent).getOS();
    os = os.name + " " + os.version;
    var browser = parser.setUA(navigator.userAgent).getBrowser();
    var browserText = browser.name + " " + browser.version;

    txt = "**Error:** " + msg + "\n";
    txt += "**Böngésző:** " + browserText + "\n";
    txt += "**Rendszer:** " + os + "\n";
    txt += "**Sor:** " + l + "\n\n";
    txt += "**Oszlop:** " + columnNo + "\n\n";
    try {
    ...
    txt += "**Melyik Menetrend:** " + streamer + "\n\n";


    if (oldJs != scriptVersion) {
        txt += "**Előző Script:** " + oldJs + " **Új Script:** " + scriptVersion;
    }
